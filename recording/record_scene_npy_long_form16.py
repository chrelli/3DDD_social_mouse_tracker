...
def sub_function_trick(which_device,top_folder):
    ...
    # open a file for writint the parameters
    with open(top_folder+'/parameters_c_'+str(which_device)+'.csv','w') as intrfile:
        writer = csv.writer(intrfile, delimiter=',')
        writer.writerow(parameternames)
        writer.writerow(parameters)
    ...
    print("extrinsics!")
    profile = cfg.get_stream(rs.stream.depth) # Fetch stream profile for depth stream
    extr = profile.as_video_stream_profile().get_extrinsics_to(cfg.get_stream(rs.stream.color)) # Downcast to video_stream_profile
    print(extr)
    print(extr.rotation)
