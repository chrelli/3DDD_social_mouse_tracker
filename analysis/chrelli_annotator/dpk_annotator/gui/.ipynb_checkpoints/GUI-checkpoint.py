...
class GUI:
    ...
    def _init_gui(self, scale, text_scale, shuffle_colors=False, refresh=100):
        ''' Initializes the GUI
        ...
        if max(self.image.shape) * self.scale < 512:
            self.scale = 512. / max(self.image.shape)
        if max(self.image.shape) * self.scale > 1024:
            self.scale = 1024. / max(self.image.shape)
        self.wasd_mode = True
        self.move_len = 1.
    ...
    def _init_skeleton(self, skeleton):
        ...
        colors = (plt.cm.hsv(np.linspace(0,1,self.n_keypoints))[...,:-1] * 255).astype(np.uint8)
        ...
        # Christian's fast HACK for nose/ears/implant -- remove this, if you use a different n of keypoints
        from matplotlib import colors as pltcolors
        color_names = ['DodgerBlue','Pink','Red','Green','Orange','Pink','Red','Green','Orange']
        for i,cn in enumerate(color_names):
            colors[i,:] = 255* np.array(pltcolors.to_rgba(cn))[:3]
        colors = np.bitwise_not(colors)
        self.colors = colors
        self.inv_colors = np.bitwise_not(colors)
    ...
    def _init_canvas(self):
        ...
        '''
        self.canvas = cv2.resize(self.image.copy(),
                                 (0, 0), None,
                                 self.scale,
                                 self.scale,
                                 cv2.INTER_NEAREST)
        empty_size = (self.canvas.shape[0],
                      int(self.canvas.shape[1] / 4.),
                      3)
    ...
    def _update_text_locs(self):
        ...
        '''
        self.text_locs = [(int(self.canvas_size[0] * 1.025),
                           int(self.canvas_size[1] * 0.05) * idx)
...
#                   
            ...
            cv2.putText(img=self.canvas,
                        ...
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=fontscale,
                        color=(int(border_color[0]), int(border_color[1]), int(border_color[2])),
                        thickness=thickness,
                        lineType=cv2.LINE_AA
                        )
            cv2.putText(img=self.canvas,
